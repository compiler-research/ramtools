...
std::vector<int64_t> RAMNTupleIndex::GetRowsInRange(int32_t refid, 
                                                     int32_t start, 
                                                     int32_t end) const {
...
RAMNTupleRecord::RAMNTupleRecord() 
    : flag(0), refid(-1), pos(0), mapq(0), 
      refnext(-1), pnext(0), tlen(0), compression_flags(kPhred33) {
    InitializeRefs();
}

void RAMNTupleRecord::InitializeRefs() {
    if (!fgRnameRefs) fgRnameRefs = std::make_unique<RAMNTupleRefs>();
    if (!fgRnextRefs) fgRnextRefs = std::make_unique<RAMNTupleRefs>();
    if (!fgIndex) fgIndex = std::make_unique<RAMNTupleIndex>();
}

std::unique_ptr<RNTupleReader> RAMNTupleRecord::OpenRAMFile(
    const std::string& filename, const std::string& ntupleName) {
...
void RAMNTupleRecord::ReadAllRefs(const std::string& filename) {
    try {
        ...
        if (!reader || reader->GetNEntries() == 0) return;
...
void RAMNTupleRecord::WriteIndex(TFile& file) {
    if (!fgIndex || fgIndex->Size() == 0 || !file.IsOpen()) return;
    file.cd();
...
void RAMNTupleRecord::Print(const char*) const {
    std::cout << GetQNAME() << "\t" << GetFLAG() << "\t" << GetRNAME() << "\t"
              << GetPOS() << "\t" << static_cast<int>(GetMAPQ()) << "\t" << GetCIGAR() << "\t"
...
std::string EncodeSequence(const std::string& seq) {
    ...
    for (size_t i = 0; i + 1 < length; i += 2) {
        encoded[j++] = (kSeqToCode[static_cast<uint8_t>(seq[i])] << 4) | 
...
void RAMNTupleConverter::ConvertSAMToRAMNTuple(const std::string& sam_file,
   ...
   while (std::getline(sam, line)) {
       if (line.empty() || line[0] == '@') {
           ...
           if (line.substr(0, 3) == "@SQ") {
               ...
               if (sn_pos != std::string::npos) {
                   ...
                   std::string seq_name = line.substr(sn_pos + 3, 
                       tab_pos != std::string::npos ? tab_pos - sn_pos - 3 : std::string::npos);
       ...
       if (!(iss >> qname >> flag_int >> rname >> pos_int >> mapq_int 
                 >> cigar_str >> rnext >> pnext_int >> tlen_int 
                 >> seq_str >> qual_str)) {
...
void RAMNTupleConverter::ConvertRAMNTupleToSAM(const std::string& ram_file,
   ...
   std::cout << "Conversion complete. Wrote " << reader->GetNEntries() 
             << " records to SAM file." << std::endl;
}
...
void RAMNTupleConverter::ViewRegion(const std::string& ram_file,
